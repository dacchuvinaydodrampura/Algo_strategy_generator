...
class StrategyReportPDF(FPDF):
    def header(self):
        ...
        self.set_text_color(50, 50, 150) # Navy Blue
        self.cell(0, 10, 'Algo Trading', 0, 1, 'L')
        
        self.set_font('Helvetica', 'I', 10)
        self.set_text_color(100, 100, 100)
        self.cell(0, 5, 'Professional Strategy Research Report', 0, 1, 'L')
    ...
    def footer(self):
        ...
        self.cell(0, 10, f'Page {self.page_no()} | Generated by Algo Trading Engine', 0, 0, 'C')

def generate_strategy_pdf(strategy: Strategy, result: BacktestResult, filename: str):
    """Generates a professional PDF report for a single strategy."""
    ...
    pdf.cell(0, 8, f"Market: {strategy.market} | Timeframe: {strategy.timeframe}", 0, 1, 'L')
    pdf.cell(0, 8, f"Report Date: {datetime.now().strftime('%Y-%m-%d %H:%M')}", 0, 1, 'L')
    pdf.ln(5)
    
    # --- Multi-Period Performance Comparison ---
    pdf.set_font('Helvetica', 'B', 14)
    pdf.set_fill_color(240, 240, 240)
    pdf.cell(0, 10, "Performance Comparison (Time Windows)", 0, 1, 'L', fill=True)
    pdf.ln(2)
    
    # Table Header
    pdf.set_font('Helvetica', 'B', 10)
    cols = [30, 35, 30, 30, 30, 35] # Column widths
    headers = ["Metric", "30 Days", "60 Days", "180 Days", "365 Days", "Total"]
    ...
    periods = [30, 60, 180, 365]
    metrics = [
        ("Net PnL", lambda r: f"{r.total_pnl:,.2f}"),
        ("Win Rate", lambda r: f"{r.win_rate*100:.1f}%"),
        ("Drawdown", lambda r: f"{r.max_drawdown_percent:.1f}%"),
        ("Trades", lambda r: str(r.total_trades)),
        ("Profit Factor", lambda r: f"{r.profit_factor:.2f}"),
    ]
    
    for metric_name, formatter in metrics:
        pdf.cell(cols[0], 8, metric_name, 1, 0, 'L') # Metric Name
        
        for period in periods:
            res = result.period_results.get(period)
            if res:
                val = formatter(res)
            else:
                val = "N/A"
            pdf.cell(cols[1 if period==30 else 2 if period==60 else 3 if period==180 else 4], 8, val, 1, 0, 'C')
            
        pdf.cell(cols[5], 8, "-", 1, 1, 'C') # Total column placeholder (optional)
        
    pdf.ln(10)
    
    # --- Performance Chart ---
    pdf.set_font('Helvetica', 'B', 14)
    pdf.cell(0, 10, "Price Action & Trade Execution (Last 30 Days)", 0, 1, 'L', fill=True)
    pdf.ln(2)
    
    # Generate Chart for the shortest significant period (e.g. 30 days) to show detail
    target_period = 30
    chart_path = None
    
    if target_period in result.period_results:
        p_res = result.period_results[target_period]
        # Fetch data for this period
        data = get_static_data(strategy.market, strategy.timeframe, target_period)
        
        if len(data.close) > 0 and p_res.trades:
            chart_path = ChartGenerator.generate_performance_chart(
                data, p_res.trades, 
                title=f"{strategy.name} - Last {target_period} Days",
                filename="temp_chart.png"
            )
    
    if chart_path and os.path.exists(chart_path):
        # Embed Image
        # A4 width is ~210mm. Margins ~10mm each. Image width ~190mm
        pdf.image(chart_path, x=10, w=190)
        os.remove(chart_path) # Cleanup
    else:
        pdf.set_font('Helvetica', 'I', 10)
        pdf.cell(0, 10, "Chart generation unavailable or no trades in this period.", 0, 1, 'C')
        
    pdf.ln(10)
    
    # --- Strategy Rules ---
    pdf.add_page() # New page for rules
    pdf.set_font('Helvetica', 'B', 14)
    pdf.cell(0, 10, "Algorithmic Logic & Concepts", 0, 1, 'L', fill=True)
    pdf.ln(5)
    
    pdf.set_font('Helvetica', 'B', 11)
    pdf.cell(0, 8, "Entry Logic:", 0, 1)
    pdf.set_font('Courier', '', 10)
    for rule in strategy.entry_rules:
        pdf.cell(10) # Indent
        pdf.cell(0, 6, f"- {rule}", 0, 1)
        
    pdf.ln(5)
    pdf.set_font('Helvetica', 'B', 11)
    pdf.cell(0, 8, "Exit Logic:", 0, 1)
    pdf.set_font('Courier', '', 10)
    ...
    pdf.set_font('Helvetica', 'B', 14)
    pdf.cell(0, 10, "Institutional Concepts Glossary", 0, 1, 'L', fill=True)
    pdf.ln(2)
    pdf.set_font('Helvetica', '', 10)
    
    # FULL GLOSSARY LIST RESTORED
    explanations = []
